...
import type { Asset, Role, Technician, WorkOrder } from "@equipment-hub/contract";
...
async function tokenFor(app: App, role: Role, technicianId: string | null = null): Promise<string> {
  await app.ready();
  return app.jwt.sign({ sub: `user-${role}-${technicianId ?? "none"}`, role, technicianId });
}

async function supervisorToken(app: App): Promise<string> {
  return await tokenFor(app, "supervisor");
}

async function technicianToken(app: App, technicianId: string): Promise<string> {
  return await tokenFor(app, "technician", technicianId);
}
...
describe("work order routes", () => {
  ...
  beforeEach(() => {
    ...
    app = createApp(repository, { jwtSecret: JWT_SECRET });
  });

  /** Injects a request as `token`'s user; defaults to a supervisor. */
  async function inject(options: InjectOptions, token?: string) {
    const bearer = token ?? (await supervisorToken(app));
    return await app.inject({ ...options, headers: { ...options.headers, authorization: `Bearer ${bearer}` } });
  }
  ...
  describe("authentication", () => {
    it.each([
      ...
      ["POST", "/api/work-orders/wo-1/assignment"],
      ["POST", "/api/work-orders/wo-1/transitions"],
    ] as const)("returns 401 for %s %s without a token", async (method, url) => {
    ...
    it("returns 401 for a token signed with a different secret", async () => {
      ...
      const token = await supervisorToken(otherApp);

      const response = await inject({ method: "GET", url: "/api/work-orders" }, token);

      expect(response.statusCode).toBe(401);
    });

    it("leaves routes outside the work-order plugin public", async () => {
  ...
  describe("authorization", () => {
    it("returns 403 when a technician cancels a work order", async () => {
      const token = await technicianToken(app, "tech-1");

      const response = await inject(
        { method: "POST", url: "/api/work-orders/wo-3/transitions", payload: { action: "cancel" } },
        token,
      );

      expect(response.statusCode).toBe(403);
      expect(response.json()).toEqual({ message: "Only a supervisor or admin can cancel a work order." });
      expect((await repository.getWorkOrder("wo-3"))?.state).toBe("triaged");
    });

    it("returns 200 when the assigned technician completes their work order", async () => {
      const token = await technicianToken(app, "tech-1");
      const transitionUrl = "/api/work-orders/wo-4/transitions";

      const started = await inject({ method: "POST", url: transitionUrl, payload: { action: "start" } }, token);
      expect(started.statusCode).toBe(200);

      const response = await inject({ method: "POST", url: transitionUrl, payload: { action: "complete" } }, token);

      expect(response.statusCode).toBe(200);
      expect(response.json()).toMatchObject({ id: "wo-4", state: "completed" });
    });

    it("returns 403 when a technician not assigned to the work order completes it", async () => {
      const transitionUrl = "/api/work-orders/wo-4/transitions";
      const started = await inject({ method: "POST", url: transitionUrl, payload: { action: "start" } });
      expect(started.statusCode).toBe(200);

      const token = await technicianToken(app, "tech-2");
      const response = await inject({ method: "POST", url: transitionUrl, payload: { action: "complete" } }, token);

      expect(response.statusCode).toBe(403);
      expect(response.json()).toEqual({
        message: "Only a supervisor, an admin, or the technician assigned to this work order can complete it.",
      });
      expect((await repository.getWorkOrder("wo-4"))?.state).toBe("in_progress");
    });

    it("returns 200 when a supervisor cancels a work order", async () => {
      const response = await inject({
        method: "POST",
        url: "/api/work-orders/wo-3/transitions",
        payload: { action: "cancel" },
      });

      expect(response.statusCode).toBe(200);
      expect(response.json()).toMatchObject({ id: "wo-3", state: "cancelled" });
    });

    it("still applies state-machine validation after authorization passes", async () => {
      const response = await inject({
        method: "POST",
    ...
    it("returns 403 when a technician assigns a technician", async () => {
      const token = await technicianToken(app, "tech-1");

      const response = await inject(
        { method: "POST", url: "/api/work-orders/wo-1/assignment", payload: { technicianId: "tech-1" } },
        token,
      );

      expect(response.statusCode).toBe(403);
      expect(response.json()).toEqual({ message: "Only a supervisor or admin can assign a technician." });
      expect((await repository.getWorkOrder("wo-1"))?.technicianId).toBeNull();
    });

    it("returns 200 when a supervisor assigns a technician", async () => {
      const response = await inject({
        method: "POST",
      ...
      expect(response.statusCode).toBe(200);
      expect(response.json()).toMatchObject({ id: "wo-1", technicianId: "tech-1" });
    });

    it("allows a technician to perform non-restricted transitions", async () => {
      const token = await technicianToken(app, "tech-2");

      const response = await inject(
        { method: "POST", url: "/api/work-orders/wo-1/transitions", payload: { action: "triage" } },
        token,
      );
  ...
  describe("GET /api/work-orders", () => {
    it("returns all fixture rows", async () => {
      const response = await inject({ method: "GET", url: "/api/work-orders" });
    ...
    it("filters by state independently", async () => {
      const response = await inject({ method: "GET", url: "/api/work-orders?state=reported" });
    ...
    it("filters by priority independently", async () => {
      const response = await inject({ method: "GET", url: "/api/work-orders?priority=medium" });
    ...
    it("returns 400 for an invalid state filter", async () => {
      const response = await inject({ method: "GET", url: "/api/work-orders?state=bogus" });

      expect(response.statusCode).toBe(400);
    });

    it("returns 400 for an invalid priority filter", async () => {
      const response = await inject({ method: "GET", url: "/api/work-orders?priority=bogus" });
  ...
  describe("GET /api/work-orders/:id", () => {
    it("returns 404 for an unknown work order id", async () => {
      const response = await inject({ method: "GET", url: "/api/work-orders/missing" });
