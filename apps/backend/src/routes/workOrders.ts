...
function isSupervisorOrAdmin(user: AuthenticatedUser): boolean {
  return user.role === "supervisor" || user.role === "admin";
}

/** Returns why `user` may not perform `action` on `workOrder`, or null when allowed. */
function transitionDenialReason(user: AuthenticatedUser, action: WorkOrderAction, workOrder: WorkOrder): string | null {
  if (action === "cancel" && !isSupervisorOrAdmin(user)) {
    return "Only a supervisor or admin can cancel a work order.";
  }

  if (action === "complete" && !isSupervisorOrAdmin(user)) {
    const isAssignedTechnician =
      user.role === "technician" && user.technicianId !== null && user.technicianId === workOrder.technicianId;

    if (!isAssignedTechnician) {
      return "Only a supervisor, an admin, or the technician assigned to this work order can complete it.";
    }
  }
...
export function registerWorkOrderRoutes(app: FastifyInstance, repository: Repository): void {
  // Encapsulated so the auth hook applies to these routes only, not to the whole app.
  app.register(async (scope) => {
    scope.addHook("onRequest", app.authenticate);
    registerRoutes(scope, repository);
  });
}
...
function registerRoutes(app: FastifyInstance, repository: Repository): void {
  app.get("/api/work-orders", async (request, reply) => {
    ...
    return await repository.listWorkOrders(filters);
  });
  ...
  app.post("/api/work-orders", async (request, reply) => {
    ...
    if (!(await repository.assetExists(body.assetId))) {
    ...
    const workOrder = {
      id: randomUUID(),
      reference: nextWorkOrderReference(await repository.references(), new Date().getUTCFullYear()),
  ...
  app.post("/api/work-orders/:id/assignment", async (request, reply) => {
    if (!isSupervisorOrAdmin(request.user)) {
      return reply.status(403).send(errorBody("Only a supervisor or admin can assign a technician."));
    }
    ...
    if (!(await repository.technicianExists(body.technicianId))) {
    ...
    await repository.saveWorkOrder(updated);
  ...
  app.post("/api/work-orders/:id/transitions", async (request, reply) => {
    ...
    const denialReason = transitionDenialReason(request.user, action, workOrder);

    if (denialReason) {
      return reply.status(403).send(errorBody(denialReason));
    }

    const result = transition(workOrder.state, action);
