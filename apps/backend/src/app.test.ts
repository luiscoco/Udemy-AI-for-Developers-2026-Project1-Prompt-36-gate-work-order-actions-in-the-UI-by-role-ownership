...
async function supervisorAuthHeader(app: ReturnType<typeof createApp>): Promise<{ authorization: string }> {
  await app.ready();
  return { authorization: `Bearer ${app.jwt.sign({ sub: "user-supervisor", role: "supervisor", technicianId: null })}` };
}
...
describe("createApp", () => {
  it("assigns a known technician and only updates assignment fields", async () => {
    ...
    const response = await app.inject({
      ...
      headers: await supervisorAuthHeader(app),
